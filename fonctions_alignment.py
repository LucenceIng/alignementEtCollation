...
# fonction pour boucleMatch sur n fichiers
def boucleMatchDossier(dossier,fichierBase,niveau):
    listeAMatcher = []
    docBase = None
    for filename in os.listdir(dossier):
        if os.path.isfile(os.path.join(dossier, filename)):
            ...
            doc = etree.parse(f)
            docDiv = XMLtoJsonParDiv(nom, doc, niveau).prodDiv()
            if nom != fichierBase:
                listeAMatcher.append(docDiv)
            else:
                docBase = docDiv
    for x in listeAMatcher:
        boucleMatch(docBase,x)
